...
def eval_model(model, dataloader, device, threshold):
    ...
    with torch.no_grad():
        for batch in dataloader:
            ...
            correct_predictions += (preds == labels).sum().item()  # cnt corrlabels
            total_predictions += torch.numel(labels)  # ttl nlabels
...
# optimizer Settings
optimizer = AdamW(model.parameters(), lr=1e-5, weight_decay=0.05)  # Experiment with 3e-5 as well
...
for epoch in range(epochs):
    ...
    train_loss = train_epoch(model, train_loader, optimizer, device, max_grad_norm=0.3)
...
similarity_threshold = 0.3  # Adjust threshold for best score predictions
